...
char getDiskDriveLetter();

int main(void) {
    char driveLetter = getDiskDriveLetter();
    while(1){
        driveLetter = getDiskDriveLetter();
        if(driveLetter!='0'){
            printf("%c \n", driveLetter);
        }
...
int main() {
    ...
    if (hDevInfo != INVALID_HANDLE_VALUE) {
        ...
        while (GetLastError() != ERROR_NO_MORE_ITEMS) {
            ...
            // deallocate it later!
            DevIntfDetailData = static_cast<PSP_DEVICE_INTERFACE_DETAIL_DATA>(HeapAlloc(GetProcessHeap(),
            ...
            if (SetupDiGetDeviceInterfaceDetail(hDevInfo, &DevIntfData,
                ...
                if (NULL != (wcsstr(reinterpret_cast<const wchar_t *>((TCHAR *) DevIntfDetailData->DevicePath), _T(
                        reinterpret_cast<const wchar_t *>("vid_10cf&pid_8090"))))) {
                    ...
                    // Eureka!
                    wprintf(_T(reinterpret_cast<const wchar_t *>("Found a device on port '%s'\n")), lpData);
                }
            }
